    ...
    <div class="container">
        <nav aria-label="breadcrumb" data-aos="fade-right">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active text-white" aria-current="page">Manage Reviews</li>
            </ol>
        </nav>
        ...
        <div class="row align-items-center mt-3">
            <div class="col-lg-8 text-white">
                <h1 class="display-3 fw-bold mb-3" data-aos="fade-right">
                    <i class="bi bi-chat-quote-fill text-warning me-3"></i>Manage Reviews
                </h1>
                <p class="lead" data-aos="fade-right" data-aos-delay="100">
                    View, edit, and manage client testimonials
                </p>
            </div>
            <div class="col-lg-4 text-end" data-aos="fade-left">
                <div class="d-flex flex-column gap-2">
                    <a href="/admin/reviews/create" class="btn btn-warning btn-lg px-5 py-3">
            ...
            <div class="col-lg-8 mx-auto" data-aos="fade-up">
                <div class="p-4 rounded-3 shadow-lg"
                    ...
                    <div class="input-group input-group-lg">
                        ...
                        @if($search)
                        <a href="/admin/reviews" class="btn btn-outline-light">
                ...
                <div class="mt-3 text-center">
                    <p class="text-white text-opacity-75 mb-0">
                        ...
                        <span class="badge bg-warning text-dark ms-2">{{ $reviews->total() }} found</span>
                    </p>
                </div>
                @endif
            </div>
        ...
        <div class="table-responsive">
            <table class="table table-dark table-hover align-middle">
                ...
                <tbody id="reviewTableBody">
                    @include('admin.reviews.table-rows', compact('reviews', 'search'))
                </tbody>
            </table>
        </div>
        ...
        <div class="mt-4" data-aos="fade-up">
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-center gap-3 p-4 rounded-3 shadow-lg"
                ...
                <div class="text-white text-center text-lg-start">
                    ...
                    Showing <span class="text-warning fw-bold">{{ $reviews->firstItem() }}</span> 
                    to <span class="text-warning fw-bold">{{ $reviews->lastItem() }}</span> 
                    of <span class="text-warning fw-bold">{{ $reviews->total() }}</span> reviews
                </div>

                <div id="paginationContainer">
                    @include('admin.reviews.pagination', compact('reviews', 'search'))
                </div>
            </div>
        </div>
        @endif
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        fetch(`/admin/reviews/search?search=${encodeURIComponent(searchTerm)}`)
            .then(response => response.json())
            .then(data => {
                document.getElementById('reviewTableBody').innerHTML = data.html;
                ...
                if (infoText && data.total) {
                    infoText.innerHTML = `<i class="bi bi-info-circle me-2 text-warning"></i>
                        ...
                        of <span class="text-warning fw-bold">${data.total}</span> reviews`;
                }
    ...
    function attachPaginationHandlers() {
        document.querySelectorAll('.pagination-link').forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                fetch(`/admin/reviews/search?search=${encodeURIComponent(search)}&page=${page}`)
                    .then(response => response.json())
                    .then(data => {
                        document.getElementById('reviewTableBody').innerHTML = data.html;
                        document.getElementById('paginationContainer').innerHTML = data.pagination;
